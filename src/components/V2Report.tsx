...
function EvidenceBadge({ classification }: { classification: string }): ReactElement {
  return <span className="rt-badge">{classificationLabel(classification)}</span>;
}
      ...
      <ol className="rt-evidence-list">
        {entries.map((entry) => (
          <li className="rt-evidence-item" key={entry.id}>
            ...
            <p className="rt-evidence-source">{entry.sourceLabel}{entry.asOf ? ` · ${entry.asOf}` : ""}{entry.sourceUrl ? ` · ${entry.sourceUrl}` : ""}</p>
          </li>
        ))}
      </ol>
    </section>
...
function Situation({ content }: { content: PersonalReportV2Content }): ReactElement {
  return (
    <section className="rt-section" aria-labelledby="rt-situation">
      <div className="rt-situation"><div><p className="rt-eyebrow">Nuläge</p><h2 className="rt-situation-headline" id="rt-situation">{content.currentSituation.summary}</h2><div className="rt-v2-stack">
        {content.currentSituation.gaps.map((gap) => <article className="rt-card rt-gap" key={gap.title}><h3>{gap.title}</h3><p>{gap.description}</p><EvidenceRefs ids={gap.evidenceIds} /></article>)}
      </div></div></div>
    </section>
  );
}
...
function Opportunities({ content }: { content: PersonalReportV2Content }): ReactElement | null {
  ...
  return (
    <section className="rt-section" aria-labelledby="rt-opportunities">
      ...
      <div className="rt-v2-grid">{content.opportunities.map((opportunity) => <article className="rt-card" key={opportunity.title}><h3>{opportunity.title}</h3><p><strong>Så fungerar det:</strong> {opportunity.mechanism}</p><p><strong>Potentiell effekt:</strong> {opportunity.potentialImpact}</p><p><strong>Rekommenderad åtgärd:</strong> {opportunity.recommendedAction}</p><EvidenceRefs ids={opportunity.evidenceIds} /></article>)}</div>
    </section>
  );
}
...
function VisualizationCard({ visualization }: { visualization: Visualization }): ReactElement {
  ...
  return (
    <article className="rt-card rt-visualization">
      <div className="rt-visual-head"><div><h3>{visualization.title}</h3><p>{visualization.description}</p></div><EvidenceBadge classification={visualization.type} /></div>
      <ul className="rt-visual-list">{visualization.data.map((datum) => <li className="rt-visual-row" key={datum.label}><span>{datum.label}</span><span className="rt-visual-value">{datum.displayValue} <EvidenceBadge classification={datum.kind} /></span>{visualization.type === "bar" && datum.value !== undefined ? <span className="rt-visual-bar"><span style={{ width: `${Math.max(0, Math.min(100, datum.value / max * 100))}%` }} /></span> : null}</li>)}</ul>
...
function ConceptCard({ concept, index }: { concept: ConceptData; index: number }): ReactElement {
  ...
  return (
    <details className="rt-concept" style={style} open={index === 0}>
      <summary className="rt-concept-summary"><span className="rt-concept-tag">Koncept {index + 1}</span><h3 className="rt-concept-name">{concept.name}</h3><span className="rt-concept-direction">{concept.artDirection}</span><p className="rt-concept-blurb">{concept.rationale}</p></summary>
      <div className="rt-concept-body"><div className="rt-concept-preview"><p className="rt-concept-eyebrow">{concept.hero.eyebrow}</p><h4>{concept.hero.headline}</h4><p>{concept.hero.subheadline}</p><div className="rt-concept-actions"><span>{concept.hero.primaryCta}</span><span>{concept.hero.secondaryCta}</span></div><div className="rt-concept-proofs">{concept.proofItems.map((proof) => <div key={proof.label}><strong>{proof.value}</strong><span>{proof.label}</span><EvidenceRefs ids={proof.evidenceIds} /></div>)}</div></div><div className="rt-concept-copy"><p><strong>Mobil:</strong> {concept.mobile.navigation} · {concept.mobile.primaryAction}</p>{concept.sections.map((section) => <article key={section.heading}><h4>{section.heading}</h4><p>{section.body}</p><span className="rt-badge">{section.layout}</span></article>)}<h4>Avvägningar</h4><ul>{concept.tradeoffs.map((tradeoff) => <li key={tradeoff}>{tradeoff}</li>)}</ul></div></div>
    </details>
  );
}
...
function Price({ price }: { price: OfferPrice | null }): ReactElement { return <>{price ? price.display : "Inte angivet"}</>; }

function OfferComponentCard({ component, label }: { component: OfferComponent; label: string }): ReactElement {
  return <article className="rt-offer-component"><header><p className="rt-eyebrow">{label}</p><h3>{component.service} · {component.tier}</h3></header><dl className="rt-tier-rows"><div className="rt-tier-row"><dt>Engångspris</dt><dd><Price price={component.oneTimePrice} /></dd></div><div className="rt-tier-row"><dt>Månadspris</dt><dd><Price price={component.monthlyPrice} /></dd></div></dl><ul className="rt-needs">{component.includedItems.map((item) => <li className="rt-need" key={item}><span className="rt-need-check">✓</span><span className="rt-need-body">{item}</span></li>)}</ul></article>;
}

function Offer({ content }: { content: PersonalReportV2Content }): ReactElement {
  const offer = content.recommendedOffer;
  return <section className="rt-section" aria-labelledby="rt-offer"><header className="rt-section-head"><div><p className="rt-section-eyebrow">Rekommenderat erbjudande</p><h2 className="rt-section-title" id="rt-offer">Datadrivet upplägg för just detta underlag</h2></div></header><div className="rt-card rt-offer"><p className="rt-reco-summary">{offer.rationale}</p><div className="rt-v2-grid">{offer.components.map((component) => <OfferComponentCard component={component} label="Rekommenderad komponent" key={`${component.service}-${component.tier}`} />)}</div><div className="rt-offer-totals"><div><span>Engångstotal</span><strong><Price price={offer.oneTimeTotal} /></strong></div><div><span>Återkommande per månad</span><strong><Price price={offer.recurringMonthlyTotal} /></strong></div></div>{offer.assumptions.length > 0 ? <div className="rt-assumptions"><h3>Antaganden</h3><ul>{offer.assumptions.map((assumption) => <li key={assumption.text}><EvidenceBadge classification={assumption.classification} /><span>{assumption.text}</span><EvidenceRefs ids={assumption.evidenceIds} /></li>)}</ul></div> : null}{offer.optionalAddOns.length > 0 ? <details className="rt-offer-details"><summary>Valfria tillägg ({offer.optionalAddOns.length})</summary>{offer.optionalAddOns.map((component) => <OfferComponentCard component={component} label="Valfritt tillägg" key={`${component.service}-${component.tier}`} />)}</details> : null}{offer.alternatives.length > 0 ? <details className="rt-offer-details"><summary>Alternativ ({offer.alternatives.length})</summary>{offer.alternatives.map((alternative) => <div key={`${alternative.service}-${alternative.tier}`}><OfferComponentCard component={alternative} label="Alternativ" /><p>{alternative.rationale}</p></div>)}</details> : null}</div></section>;
}

function StringList({ id, eyebrow, title, items }: { id: string; eyebrow: string; title: string; items: string[] }): ReactElement | null {
  if (items.length === 0) return null;
  return <section className="rt-section" aria-labelledby={id}><header className="rt-section-head"><div><p className="rt-section-eyebrow">{eyebrow}</p><h2 className="rt-section-title" id={id}>{title}</h2></div></header><ul className="rt-needs">{items.map((item) => <li className="rt-need" key={item}><span className="rt-need-check">✓</span><span className="rt-need-body">{item}</span></li>)}</ul></section>;
}

export function V2Report({ content }: { content: PersonalReportV2Content }): ReactElement {
  return <>
    <ReportHero companyName={content.companyProfile.companyName} contactName={content.companyProfile.contactName} title={`${content.companyProfile.companyName} · personlig analys`} introduction={`${content.companyProfile.industry} i ${content.companyProfile.location}. Målgrupp: ${content.companyProfile.audience}.`} highlights={content.companyProfile.brandTraits} />
    ...
    <p className="rt-disclaimer">{content.disclaimer}</p>
  </>;
}
